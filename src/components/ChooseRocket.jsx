...
export default function ChooseRocket() {
  ...
  useEffect(() => {
    async function getRocket() {
      try {
        const getRocket = await fetch(`http://localhost:4000/rocket`, {
          method: "GET",
          mode: "cors",
          headers: {
  ...
  return (
    <>
      {rocket ? (
        ...
        <div role="status " className="scale-[3]">
          <svg
            aria-hidden="true"
            className="w-8 h-8 mr-2 text-gray-200 animate-spin dark:text-purple-300 fill-blue-800"
            viewBox="0 0 100 101"
            fill="none"
